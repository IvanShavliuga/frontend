...
if ($_POST) { // eñëè ïeðeäaí ìaññèâ POST
	$name = htmlspecialchars($_POST["usr"]); // ïèøeì äaííûe â ïeðeìeííûe è ýêðaíèðóeì ñïeöñèìâoëû
	$email = htmlspecialchars($_POST["email"]);
	$subject = htmlspecialchars($_POST["number"]);
	$message = htmlspecialchars($_POST["message"]);
	$json = array(); // ïoäãoòoâèì ìaññèâ oòâeòa
	if (!$name or !$email or !$subject or !$message) { // eñëè õoòü oäío ïoëe oêaçaëoñü ïóñòûì
		$json['error'] = 'Âû çaïoëíèëè íe âñe ïoëÿ! oáìaíóòü ðeøèëè? =)'; // ïèøeì oøèáêó â ìaññèâ
		echo json_encode($json); // âûâoäèì ìaññèâ oòâeòa 
		die(); // óìèðaeì
	}
	if(!preg_match("|^[-0-9a-z_\.]+@[-0-9a-z_^\.]+\.[a-z]{2,6}$|i", $email)) { // ïðoâeðèì email ía âaëèäíoñòü
		$json['error'] = 'Íe âeðíûé ôoðìaò email! >_<'; // ïèøeì oøèáêó â ìaññèâ
		echo json_encode($json); // âûâoäèì ìaññèâ oòâeòa
		die(); // óìèðaeì
	}
	...
	$emailgo->from_email= 'dontforget.pro'; // oò êoão
	$emailgo->from_name= 'Òeñòoâaÿ ôoðìa';
	$emailgo->to_email= $email; // êoìó
	$emailgo->to_name= $name;
	$emailgo->subject= $subject; // òeìa
	$emailgo->body= $message; // ñooáùeíèe
